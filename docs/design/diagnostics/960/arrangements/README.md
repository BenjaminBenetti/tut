...
Current integration passes typecheck, full lint/format, build and **2,317 unit tests** (231.82 s, exit 0; one optional wide skip). All seven current-head simulation checks pass (192.52 s, exit 0; one optional skip). Browser shards 1–3 pass with clean exits (14 / 23 / 14 tests; 12 / 3 / 12 optional skips). Shard 4 was terminated with exit 143 before completion and will run again after captures close. Separate repeated integration frames are being recorded before this PR leaves draft. The first integrated wide invocation failed its unchanged 600-second budget (642.39 s wall, timeout only); it ran alongside software rendering. An isolated rerun follows after those browsers close. This is not yet a green current-head wide verdict, and no timeout is raised.
