...
Subsequent [real-map navigation evaluations](../experiments/jev-navigation/README.md) expose the limits of those smoke checks: the current movement question reached none of six held-out objectives under faction fog. A prototype that asks Jev to choose an objective and uses game pathfinding plus limited exploration reached all six, using 379 input tokens per request. The record includes an interactive route viewer, complete traces, failed simplifications and an opaque-door candidate gap. The prototype lives in the eval tools; it has not changed the runtime controller.
