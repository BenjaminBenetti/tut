...
- **Other destinations:** visible tech carcasses, position-only radar contacts and unconfirmed last-seen locations.
...
| `move_to_extraction` | TDF move into the nearest reachable extraction tile to leave. Arrival on the last AP completes free extraction after the walk; with AP remaining, choose Extract next. Bugs can approach but cannot extract. |
| Carcass target | Move within harvesting range of a visible, unharvested carcass. Offered only to actors able to harvest; harvesting is a separate action. |
| Radar contact | Investigate a position-only contact to gain sight. No hidden identity, health or attack target is disclosed. |
| Last-seen location | Investigate a historical sighting. The entity may have moved or died; a location already in sight needs no investigation. |
...
Jev reads weapons and usables from each entity’s actual loadout, for both factions, using the game’s normal legality checks and previews. New content using existing mechanics needs no Jev-specific wiring. New action or item kinds must supply targeting, execution and gameplay instructions; typechecking prevents silent omissions. Destination sources have the same requirement: each needs a movement provider and a coverage case.
