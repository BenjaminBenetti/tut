...
The default devcontainer has no required GPU device. The [iGPU variant](../../.devcontainer/README.md)
passes only `/dev/dri/renderD129` (AMD Granite Ridge, `1002:13c0`). `renderD128`
is the discrete RX 9070 and must remain absent inside either container. Both
configurations share the image with container-local RADV and Vulkan/EGL diagnostics.
No host configuration changes.

The Director coordinates fleet rebuild timing and selects the iGPU variant on
the studio host. [Post-rebuild paired measurements](https://github.com/BenjaminBenetti/tut/issues/1069#issuecomment-5629783197)
verify RADV/radeonsi access and reduced rendering CPU cost; evidence stays software.
