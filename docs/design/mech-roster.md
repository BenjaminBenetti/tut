...
| 2 | Jumper* | Short jumps over obstacles and suitable elevation changes; power and heat intensive. |
...
| 2 | Thermal Lance | Short-range energy attack against tough individual targets; extreme heat. |
...
- Jumping is an explicit action with a preview, valid outdoor landing, bounded distance and height, heat and action costs. It never grants access to infantry-only interiors.
...
- Bracing trades an action and mobility for stability. Moving or jumping releases it. Stationary bonuses reset after moving and at the appropriate turn boundary.
