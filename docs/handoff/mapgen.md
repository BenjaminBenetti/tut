...
Last updated: 2026-09-06 (Astra 6 seat; #936 artificial-plinth withdrawal ready for frame judgment). Read this entry before the historical notes.
...
- **#936 is active**, branch `fix/936-remove-vegetated-plinths`, baseline `2878dfc`. This Executive Director ruling explicitly overturns #910's planted-bed preservation instruction. Cause posted before implementation in issue comment `5562738987`: `ElevationPass` stamps a storey-high rectangle and rails its edges; later props add vegetation. The four remaining non-road families now have `maxPerMap: 0`: terrace, raised-park, rail-embankment and rubble-mound. Rock-topped stamps are included because they are the same artificial blocks (the embankment has no railway). The #910 planning/restoration algorithm and proposal weights remain unchanged; withdrawn plots regain original height/surface with no replacement family.
- **#936 evidence:** `docs/design/diagnostics/936/README.md`, eight inspected Map Lab frames, before/after frequency JSONL, paired preservation records and simulation reports. Across 108 opening-seed recipes: **213 soil/grass beds → 0**, all **298 artificial plinths → 0**, restoring **10,437 columns**. All 108 raw terrain hashes match; all 36 rural and 36 town complete map hashes match. City building records and 42,071 upper-floor tiles are unchanged. **Tactical cost:** mech-reachable outdoor tiles above median road grade fall 9,271 → 5 (30/36 city maps with any such ground → 1/36). Do not claim building height compensates for mech outdoor height. The critic must assess that loss; do not put mounds back.
- **Town acceptance discrepancy remains explicit:** shipped town presets have no elevated-feature budget, hence zero artificial beds even before #936. Director's intended town recipe was requested in-session and on #936; no reply at preparation. `coast-control-12` is an honest unchanged town control, not a removal example. Its prominent railed bank at x=32–33,z=13–17 is natural layer 2 throughout the pipeline. Two smaller railed columns at x=40–41,z=23 are a graded building-lot margin, a different cause. Do not manufacture a town bed by turning on a dormant knob or remove natural hills to satisfy the picture.
- **#936 validation:** typecheck/lint/build pass; 2,167 unit and 59 browser tests pass; seven simulation checks pass, 45/60 → 46/60 wins, 24/24 at difficulties 1–4 unchanged. New stock-preset regression failed on the old catalogue and passes with the caps; existing feature mechanics remain exercised through explicitly enabled fixtures. Only the city golden changes to `4220186506`. The required wide sweep passes all 1,200 maps with zero hook relocations (366 s). Director frame judgment remains outstanding.
- **#910 is accepted and merged** as #926 (`fd032fd`); its planted-bed requirement is superseded by #936. **#915 is accepted and merged** as #932 (`121f397`): coastal street ends become existing-grade pedestrian aprons with a water-edge rail. Its broad water-strip layout concern is separate and already stated on #915. Do not reopen either delivered ticket.
- **Next assigned work:** check the live assigned list before parking after #936. #911 is Art's dropship with a MapGen envelope/placement agreement needed; #917 is an evidenced p2 rural-fence placement issue queued after it. Neither may pre-empt the active Director p1. Read the current issue bodies/comments before any build.
...
- **Watch/process:** one bounded five-minute REST loop, three-hour hard stop, no cron or second loop. Poll newly created/relabelled `area:mapgen` issues and owned PR review/merge events; the local pending-event file must be consumed before rearming. A completed PR leads to an explicit assigned-queue check, not automatic waiting. Update local work state to the new issue/PR/head before arming; never let an old merge event suppress queued work.
