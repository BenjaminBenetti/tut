...
- All ordinary action families use existing rules. Every decision starts with a compact Choice built from the actor's available abilities: each weapon and firing mode, each usable item, and other actions such as move or overwatch. The next Choice contains only targets/destinations for that exact weapon/mode or item. Large families use region/target groups with at most 32 routing choices, then detail pages with at most 48 options and 12,000 criteria characters (a single indivisible action is retained). Every candidate remains reachable. This bounds enumeration size as well as option count; these are application budgets, not an exact tokenizer. The model chooses an existing candidate, never executable code or invented coordinates.
