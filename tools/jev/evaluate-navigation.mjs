...
async function episode(world, variant, trial) {
  ...
  for (let step = 0; step < 4 * run.apCap && run.apUsed < run.apCap; step++) {
    ...
    if (!candidates.length && variant !== "goal-route-doors") {
...
for (const definition of caseDefinitions(suite).filter(
  (def) => !selected.length || selected.includes(def.id),
)) {
  const world = buildCase(definition, visibility);
  result.cases.push(world.metadata);
  await writeFile(
    join(output, "maps", `${definition.id}.json`),
    JSON.stringify(world.mission.map),
  );
  console.log(JSON.stringify({ case: definition.id, ...world.metadata }));
  if (!dry)
