...
try {
  for (
    let repeat = 0;
    repeat < Number(process.env.CAPTURE_REPEATS ?? 2);
    repeat++
  ) {
    ...
    try {
      ...
      for (const [
        ...
        const clip = {
          x: Math.max(0, Math.min(900, Math.round(info.pixel.x - 750))),
          y: Math.max(0, Math.min(400, Math.round(info.pixel.y - 550))),
          width: 1500,
          height: 1100,
        };
    ...
    JSON.stringify(
      {
        ...
        repeatedBrowsers: Number(process.env.CAPTURE_REPEATS ?? 2),
        records,
      },
      null,
      2,
