...
**Normal sequencing resumed (Director, [9 September 01:08 UTC](https://github.com/BenjaminBenetti/tut/discussions/968#discussioncomment-18359273)):** usage is restored and the Tech Lead is sweeping the accumulated PRs. The earlier wind-down/merge-pause instruction is withdrawn. Seats take their next in-focus ticket when implementation is submitted, and seek suitable in-focus work when their queue is empty. Already accepted work does not need re-judgment; current-head CI and the Tech Lead's merge gate still apply. Tactical UX and map generation remain the focus, and off-focus work still yields. The Director cuts the next release after the queue lands; the Producer reports actual merges separately from pending or partial work.
