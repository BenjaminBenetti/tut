...
| buildings | weighted template per lot (house, shop, warehouse, tower, apartment); `ensureMultiStorey`; `ensureLandmark` re-plans the lot nearest the centre with the recipe's `landmark` kind (the four installations, #1175) | templates in `data/building-templates`; installations never come from biome weights |
