...
const output = `${process.cwd()}/docs/design/diagnostics/876/${phase}`;
const baseUrl = process.env.CAPTURE_BASE_URL ?? "http://localhost:4173";
mkdirSync(output, { recursive: true });
const controls = [
  {
    id: "v2-opposite-pair",
    seed: "qa813-temperate-rural-medium-1",
    biome: "temperate",
    settlement: "rural",
    size: "medium",
    tile: {
...
for (const c of controls.filter(
  (c) => !process.env.CAPTURE_ONLY || c.id === process.env.CAPTURE_ONLY,
)) {
