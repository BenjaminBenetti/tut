...
Jev first chooses a **specific available action** from the entity's current loadout: for example, **Attack with Carbine**, **Attack ground with Autocannon**, **Use Grenade**, **Use Medkit**, move or overwatch. Each weapon, firing mode and usable item has its own top-level choice, with its name and capability. When needed, a follow-up selects only targets or tiles for that chosen weapon/mode or item. A single concrete action, such as overwatch, executes without a redundant target question. The menu is rebuilt from currently legal actions, so depleted items and weapons without a legal shot disappear. Large lists narrow through region or target groups first, keeping all legal candidates reachable. Detail pages are bounded by both option count and description size.

There is no `finish` choice. Movement offers named entities and objectives, `move_north`, `move_east`, `move_south`, `move_west`, and `move_away_from_enemies`. The game prepares each route using faction knowledge and caps the proposed movement at **one AP**. Unavailable or already-reached destinations are omitted. Retreat maximizes distance from the nearest known hostile footprint; compass directions use north = −z, east = +x, south = +z, west = −x. After an action, automatic play captures the updated position, vision and remaining AP and asks again while the entity can still act. Other actions retain their normal game costs, including actions that consume all remaining AP.
...
Tile maps and exhaustive movement tile menus are no longer sent to Jev. Pathfinding stays in the game and respects occupancy, footprints, walls, elevation and weighted terrain costs. For a known destination without a complete known route, it can approach a reachable exploration boundary; it never reads hidden terrain to choose that route. Remembered geometry can be stale, so the authoritative command handler validates again before moving. Opaque doors or disconnected knowledge can still block progress; this is not a claim of solved fog exploration.
...
The inspector pauses independently before the destination and distance questions. No move is ready to execute until distance evaluation succeeds; output includes the normalized fraction, rounded cost, endpoint and actual command. The same three stages run automatically in gameplay.
