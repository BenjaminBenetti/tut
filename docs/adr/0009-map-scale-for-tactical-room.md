...
| Elevated feature footprints, hooks, densities per area | – | unchanged | Features are placed by attempts against the plat and already scale with it; hook distances and per-100-column densities were re-verified across the sweep, not retuned (balance is out of scope, #826). |
