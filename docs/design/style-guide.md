...
opaque prints with this palette: a trade color and pictogram, and a business name
chosen from 50 authored names per type, with icon-only versions for narrow entrances. Their geometry,
...
- **Emphasis is a claim, and it has to be true.** Accent colour, a danger border, larger type and a primary button all assert something — *this is the recommended action*, *this is what went wrong*, *this is the state of the world*. Applied by position or applied unconditionally, they assert it whether or not it holds, and a player believes the loudest thing on the screen before they read it. Three of these shipped and were found in one afternoon:
  - The debrief gave "Mechs destroyed" a danger border and 1.15em type **whatever the section contained**, so a clean mission opened with a red alarm reading "No mechs lost." (#736).
  - The event dialog made the **first** choice primary, and on three of the four event types the first choice is not the default — so it recommended an action next to a line saying a different one happens if you do nothing (#742).
  - The game-over screen inherited the title screen's bright world, so "EARTH OVERRUN" was announced over a pristine, thriving Earth (#749).

  The test is simple: **read the emphasis aloud as a sentence and check the data agrees.** Where it does not, the fix is usually to make the emphasis conditional (#736), or to replace it with information — the event dialog now names the default rather than recommending one.
...
| Complete infested host variant (including original object) | ≤ 6 500 | ≤ 250 KB |
| Legacy resin vertical roots / prop collar | ≤ 1 700 / 450 | ≤ 100 KB / 60 KB |
| Legacy resin nursery wall / prop organ / floor detail | ≤ 6 000 / 4 000 / 750 | ≤ 200 KB / 150 KB / 60 KB |
...
The approved **Resin Shell** direction (#1166, 2026-09-17) now uses complete infested versions of city models in three stages, following the user's rejection of clay-like brown overlays. Wet tendons follow each host's geometry; perforated membranes, embedded larvae and breathing throats occupy recesses, bark and fan wells. Original masonry, paint and foliage remain visible. The catalogue contains 21 hosts × three stages. Thin connected ground strands and sparse seeps replace broad sheets; walkable growth stays below 0.08 units. Dark tissue uses smooth normals, low roughness, a glossy coat and a shared sky reflection scoped to resin. Wall/door/window geometry preserves height, sockets and apertures. Stage selection preserves fog, floor-cut and demolition ownership. At level 10, 98% of eligible tiles are marked, with established and consumed hosts mixed together. See the [Resin Shell kit](kits/resin-infestation.md) for sources and actual Map Lab captures.
...
**It is a window where the unit is, not a see-through building.** Since #1134 the cutaway is a bundle of rays from the unit to the camera: from the four corners of its footprint at foot and head height, from its spine at the feet, the waist and the head, and since #1138 from a ring of eight points at waist height `GHOST_RING_MARGIN` (0.75 u) outside the footprint. A fragment fades only where it stands *on* one of those rays — nearer the camera than the sample and within the ray's radius of the sample's spot on the view plane — so what gives way is a cone around the unit: the roof and the walls over about a two-tile ring of its floor. A wall in front of the unit but three tiles to its side stays solid; so does a wall well behind it (farther than every sample, the ring's back included) and the floor under it (#1118). Before this the test was a plane through the unit with a four-tile disc around it (#1132), and everything on the camera's side of that plane within the disc faded, walls beside the unit included (Executive Director, 2026-09-13, #1134). The body rays alone, at a 0.6 radius, opened a window the shape of the unit's silhouette — the unit showed and nothing it could walk to did — which the Executive Director found too tight to navigate a room by; the ring and the 1.2 radius replace it, judged on [a squad inside a generated building](ghost-interior.png) and [the test room](ui-ghost-depth.png) (Executive Director, 2026-09-14, #1138).
...
The case to check is a squad behind building geometry, with the surrounding map still present. The #937 comparison above has two squads in a generated house; its linked report also covers the existing flat-roof apartment and views 180° apart.
...
| Ray radius | **1.2 tiles** around each sample point, across the view plane | With the waist ring 0.75 u outside the footprint, the cone reaches about two tiles out from a one-tile unit, so the tiles it can step to and the near walls read through the roof, and a wall three tiles off is outside every ray (#1138). The 0.6 of #1134 — the corners' rays just overlapping into one silhouette — showed the unit and nothing around it. The 4-tile disc the Executive Director chose in #937 was the radius of the old plane rule; the rays replace it. |
...
**Inspection (#1023).** Use the layer controls to look inside an empty building.
Pointer position does not open a reveal; unit ghosting retains the settings above.
