...
- **Objectives and mission types are rule tables** ([ADR 0013](../adr/0013-campaign-progression.md) §2.3): outside the kind and type modules themselves, tactical services never switch on an objective kind or a mission type. `tactical/model/objective-rules.ts` defines `ObjectiveRules<K>` (`complete`, `failed`, and optional `interaction`, `phaseStep`, `reachable`, `marker`, `destination`, `onDeadline`, `tally`, `resultFields`); `OBJECTIVE_RULES` in `tactical/service/objectives/` maps each kind to its own module, and a missing or misfiled kind is a compile error. `MISSION_SETUP_RULES` in `tactical/service/missions/` does the same for what each `MissionTypeId` puts on a fresh map, and reaches `startTacticalMission` through `MissionStartDeps.setupRules`. A story mission may add to its type's map: `STORY_SETUP_RULES` in `tactical/service/story/` (a `Partial` table keyed by `StoryMissionId`, `MissionStartDeps.storySetupRules`) runs right after the type's rule, on its result (ADR 0013 §2.5). An objective marked `optional` does not decide the mission: `decidingObjectives` drops it, and the outcome, the leave summary and the tracker's count read only what is left (#1179). Every objective may carry `deadlineTurn`; the generic deadline phase step fails it once that turn has ended, announces `ObjectiveUpdated` and runs the kind's `onDeadline`. The tactical resolver writes one generic `ObjectiveResult` row per objective to `MissionResult.objectives`. A new kind or type is a new module and one table entry.

  ```
  startTacticalMission ──► MISSION_SETUP_RULES[mission.typeId].setup ──► objectives, spawners, generators
                       ──► STORY_SETUP_RULES[mission.storyId]?.setup ──► the story's own (Live Specimen:
                                                                         nests optional, capture, lurkers)
                                                                             │
  objective ──► OBJECTIVE_RULES[objective.kind] ◄────────────────────────────┘
  missionOutcome / objectivesComplete ──► decidingObjectives (optional ≠ true) only
...
- **The overworld offers missions through a director over two more tables** ([ADR 0013](../adr/0013-campaign-progression.md) §2.3–2.5, campaign arc §5). Each `MissionTypeId` has one entry in `MISSION_OFFER_RULES` (`overworld/service/missions/`). An entry is either an offer rule (`debut`, `eligible → MissionSite[]`, `create`) that the director draws, or a trigger rule (`trigger → Mission[]`) that fires on its own condition; Defend Installation and Hive Assault are trigger rules. A trigger rule may also carry `refresh(offer, state, ctx) → Mission | undefined`, which the director runs over that type's standing offers before anything else each day: Hive Assault re-levels its pinned offer through it, and withdraws it once the hive is gone. The type's entry in `MISSION_CONSEQUENCE_RULES` says what playing the offer (`onResolved`, called by the launch handler) or letting it lapse (`onExpired`, called by `expireMissions`) does to the map. The `mission-generation` tick step is the director:
  0. It refreshes the standing offers whose trigger rule has a `refresh`.
  1. It runs the pin triggers first, each on its own RNG fork. The story pin trigger pins each built story mission whose act and flags are due. When none of a story's candidate cities is free, `pickStoryCity` takes the preferred one holding an ordinary offer. The director withdraws that offer (`MissionWithdrawn`, no expiry, no consequence) and never withdraws a pinned or triggered one (#1179).
...
- **Act III's story missions are story defences** (#1179; campaign arc §4, §6.9). Uplink (`overworld/service/story/uplink.ts`) and Launch Window (`launch-window.ts`) are `defend-installation` offers built by `buildStoryDefenceOffer` (`story-defence-offer.ts`). The facility is the story's, not a built installation, so `InstallationDefence.installation` is an `InstallationSiteId`: a `DeployableTypeId` or a `StoryInstallationId` (`tracking-array`, `launch-site`). `deployableId` is absent. `INSTALLATION_SITES` names each facility and gives its `compound`, the authored mapgen site it raises. The tracking array borrows the sensor array's yard (2 generators) and the launch site the repellent dispersal plant's (4), so neither needs a new layout, and the defend map rule, setup, objective tracker and debrief name them through the same table. `storyDefenceCity` picks the site without a draw, through `pickStoryCity` over every city: the least infested detected city in a region without a hive, else the least infested detected city, else any city, and a free one first. Only when no city is free does it take an ordinary offer's city, which the director withdraws. A Defend Installation or a pinned offer is never taken. Uplink (d6, 5 waves) pins as soon as the campaign is in Act III and sets `uplink-won`, which reveals Intel III, Platform Approach (`tech.platform-approach`, 280 TP, sets `platform-approach`). Launch Window (d8, 7 waves) pins once `platform-approach` and `great-hives-destroyed` are both set, and its win is `advance-act`: the finale once the Spore Platform is built, campaign victory until then. A loss of either re-pins it five days later. Last Hope (`tech.last-hope`, 100 TP) shows once `platform-failed` is set and sets `last-hope`, which D7 reads to pin the platform again. Both nodes are in `tech/data/endgame-intel-nodes.ts`, on the support spoke beside Intel I. The briefing and debrief come from `STORY_PRESENTATION`: `uplink-presentation.ts` and `launch-window-presentation.ts` (`ui/service/story/`) share `story-defence-presentation.ts`. It writes the "Hold the <facility> through N waves" row, and it knows a story defence's result by `result.defence.installation` (`UPLINK_SITE`, `LAUNCH_WINDOW_SITE`) and the story's record in `CampaignProgress`. An auto-resolved result records no defence, so it keeps the outcome's line. See [`uplink-offer.png`](uplink-offer.png), [`uplink-briefing.png`](uplink-briefing.png) and [`platform-approach-node.png`](platform-approach-node.png), the node the day after Uplink is won, all captured by `tools/ui/capture-uplink.mjs`.

  ```
  act-3 ──► Uplink (tracking array, d6) ──won──► uplink-won ──► Platform Approach shows ──► platform-approach ─┐
                                                                  last Great Hive falls ──► great-hives-destroyed ─┴► Launch Window (launch site, d8)
  Launch Window won ──► advance-act ──► finale (if built) or campaign-won
  either lost ──► storyRetryDay = day + 5 ──► pinned again
  ```
...
- **Live Specimen ends Act I** (#1179; campaign arc §4, §6.9). `overworld/service/story/live-specimen.ts` pins a d3 infestation clearance once `capture-net` is set (Pheromone Analysis), at the worst detected infested city: a free one if any, otherwise the worst holding an ordinary offer, which is withdrawn. So it pins the day after the research (arc D2). It never expires. A win runs the clearance's own consequences and then `advance-act`: Act II if Intact Pod is built, otherwise `campaign-won`, which is victory on the next tick. A loss or an extraction re-pins it five days later. Its `StorySetupRule` (`tactical/service/story/live-specimen-setup.ts`) keeps the nests as optional objectives and adds `capture-specimen` for the lurker as the one that decides. It stands two lurkers by the nests nearest the deploy zone, on ground the squad can walk to. A delivered specimen wins with every nest still standing. Killing every lurker does not fail it, because a clearance's edge waves never stop. The briefing and debrief come from `STORY_PRESENTATION` (`ui/service/story/`): story rows ahead of the type's, the story's own description, and a tagline asked before the type's. See [`live-specimen-offer.png`](live-specimen-offer.png) and [`live-specimen-briefing.png`](live-specimen-briefing.png), captured by `tools/ui/capture-live-specimen.mjs`.

  ```
  capture-net ──► pin (d3 clearance, worst detected infested city) ──► start: clearance setup ──► live-specimen setup
  netted lurker carried aboard ──► capture complete ──► won ──► storyWon + advance-act ──► campaign-won ──► victory
  extracted / lost ──► storyRetryDay = day + 5 ──► pinned again
  ```
...
- **The hive core is a 3×3 spawner variant** (#1179). Hive Assault's core is a `Spawner` with `variant: "hive-core"`, like the spore pod, so shots, blasts, charges and fire already reach it. `SPAWNER_VARIANT_TRAITS` gives it `footprint: 3`, anchored at its lowest `x` and `z` like a unit's (#1130), and `footprint-service.ts` answers for every tile: `occupiedKeys` holds all nine so nobody walks through it, reach and charges measure to its nearest tile (`nearestSpawnerTile`), a blast that covers several of its tiles strikes it once, and `perceivedOccupantAt` finds it on any of them. `setUpHiveAssault` (`tactical/service/missions/hive-assault-setup.ts`) stands the core on the cavern's `hive-core` hook with `hiveCoreHp(level)`, the nests on the `egg-spawner` hooks with a tech-point `bounty`, the `destroy-hive-core` objective, and `hiveGuardCount(level)` Hive Guards on `hiveGuardPositions(map)`; it ends with a `HiveBroodPlacement` seam for the dormant broods. The shipped rule passes `NO_BROODS`; `createHiveAssaultSetup(placeCavernBroods)` fits the seam as it stands (a test proves it on a generated cavern), so waking the cavern's broods is that one line in `mission-setup-rules.ts`. The objective's flag is set by `damageSpawner` when the core falls, but `complete` also needs `mission.extracted` to be non-empty, so a core killed by a force that never gets home is not a win. The resolver pays destroyed spawners' `bounty` through `techPointsFor`, so a loss pays none. Graphics draws spawners at their footprint's centre and swaps `bug.hive-core` for `bug.hive-core-damaged` below half its hit points through `SPAWNER_MODELS[variant].damaged`.

  ```
  offer (pinned, hive {level})  ──► hive cavern (MISSION_MAP_RULES, nests by level)
  setup  hive-core hook ──► core 3×3, hp 60 + 15·level
         egg-spawner hooks ──► nests, bounty 5 TP each
         destroy-hive-core ──► objective      hiveGuardPositions ──► 2–4 Hive Guards
  play   damageSpawner(core) ──► objective.complete = true (the core fell)
         complete(objective) = objective.complete && extracted.length > 0
  result hiveCoreDestroyed, techPointsBounty ──► HIVE_ASSAULT_CONSEQUENCE
         won ──► liberateRegion + hive-core-sample (first win)   else ──► the hive stands
  ```

- **Sitreps are rolled onto the offer and applied from a third table** (campaign arc §11, #1179). The last offer decorator, from `overworld/service/missions/sitrep-offer.ts`, fills `Mission.sitreps` from campaign mission 10 on (`missionsPlayed + 1`). It uses `ACTS[act].sitrepSlots` slots at `sitrepChance` each, and draws only sitreps that have debuted, that the offer's mission type can host (`SitrepDefinition.requiredHooks`, each a hook kind the type places), and that the offer does not already carry. Story offers carry none. The list is then frozen on the offer. Tactical copies it to `TacticalState.sitreps`. `SITREP_RULES` in `tactical/service/sitreps/` maps each `SitrepId` to a `SitrepRule` with three optional parts:
  ...
  Setup runs after the garrison, so a sitrep sees every unit and objective it must keep clear of. The first look runs last and adds to what a sitrep already revealed. The UI reads names and effect lines from `ui/data/sitrep-presentation.ts`: one tag row per sitrep on the briefing, and compact tags on the mission list row. See [`sitreps-briefing.png`](sitreps-briefing.png). A presentation can also carry a `deadline` (a phrase and the turn it reads off the mission). `sitrepCountdowns` in `ui/service/objectives/` turns each running one into a row under the objective tracker's objectives, and the turn banner shows the soonest of the objectives' and the sitreps' countdowns. Dust-off Window uses this; see [`sitrep-dustoff-hud.png`](sitrep-dustoff-hud.png). Its phase step strands the force through `leaveBehind` (`tactical/service/left-behind-service.ts`), which the Leave command's abandon uses too.

  ```
  startTacticalMission ──► recipe ──► map ──► deployment ──► base state (bugMix, sitreps copied)
                       ──► MISSION_SETUP_RULES[typeId].setup ──► STORY_SETUP_RULES[storyId]?.setup
                       ──► garrison (fork garrison-turrets)
