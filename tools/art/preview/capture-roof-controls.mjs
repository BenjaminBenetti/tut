...
const output = `${process.cwd()}/docs/design/diagnostics/916/${phase}`;
...
for (const c of controls) {
  ...
  if (c.units === 0) {
    ...
    await page.screenshot({
      ...
      clip: { x: 380, y: 0, width: 2020, height: 1500 },
    });
    records.push({
      ...c,
      url: page.url(),
  ...
  if (c.maxLevel !== undefined) {
    await page.locator("#level").fill(String(c.maxLevel));
    await page.locator("#level").dispatchEvent("input");
  }
  ...
  for (let i = 0; i < 24; i++) {
    ...
    if (Math.abs(pitch - 55) < 0.8) break;
    await page.mouse.wheel(0, -Math.log(55 / pitch) / 0.0015);
    await frame();
  }
