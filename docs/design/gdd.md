...
- **Infested ground** costs player units **2 movement points** per tile and bugs **0.5**, against the usual 1. Pathfinding, movement overlays, command validation, AP spending and bug AI all use these costs. A multi-tile unit uses the most expensive destination cell: bugs gain the bonus when their whole footprint is on growth. Nests provide high cover, block sight and movement, and can be demolished. From level 6, suitable large colony cores can support enclosed carapace lodges and brood halls; level 8 also allows armoured keeps. These occupy reserved multi-tile sites with clear circulation margins, provide high cover and block elevated sight through their shell heights. They are solid demolishable structures without walkable interiors or roofs. Map Lab exposes the 0–10 infestation slider and saves it in shared URLs.
