...
```text
                         ...
                         |
mission --> observation + action catalogue --> action-type Choice --> relay --> Jev
                         |                                         |
                         +--> Choice within selected type <--------+
   ^                                                               |
...
- Navigation is a full-width/depth ASCII map for each elevation layer, with an explicit coordinate convention and legend. Glyphs distinguish current observations, stale memory and unknown cells, with passability resolved for the actor's movement class. A fog fill covers omitted rows and padding; row offsets place each ASCII span at its exact global coordinates. Every known cell is retained. Marker records retain entity/objective identities and the terrain under them; public markers never uncover hidden surfaces. Separate ASCII edge maps, grouped elevation connectors and exceptional cover/opacity properties preserve geometry that a single tile character cannot express.
- Every request explains the game's AP, health, turn, knowledge and objective rules. Action-type options carry their own instructions and actual AP costs; follow-ups repeat the selected weapon/item capability and action rules without relying on prior conversational memory. Move choices include weighted path cost and step count, and explain that unused one-AP movement range is lost. The inspector and automatic controller share these descriptions.
- All ordinary action families use existing rules. Every decision starts with a compact Choice built from the actor's available abilities: each weapon and firing mode, each usable item, and other actions such as move or overwatch. The next Choice contains only targets/destinations for that exact weapon/mode or item. Large families use region/target groups with at most 32 routing choices, then detail pages with at most 32 options and 8,000 criteria characters (a single indivisible action is retained). Every candidate remains reachable. This bounds enumeration size as well as option count; these are application budgets, not an exact tokenizer. The model chooses an existing candidate, never executable code or invented coordinates.
