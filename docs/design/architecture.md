...
- The strategic map's Earth is data, not art (#1144): `graphics/data/earth-coastlines.ts` holds Natural Earth's land polygons as closed `[lon, lat]` rings, rebuilt by `tools/art/build-coastlines.mjs`; `graphics/view/earth-wireframe.ts` draws them as glowing vector coastlines over a graticule, through the same equirectangular projection the city layout uses, so continents and markers agree by construction.
