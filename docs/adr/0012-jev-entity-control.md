...
- All ordinary action families use existing rules. Every decision starts with a compact Choice of action types. The next Choice contains only actions of the selected type. Large families use region/target groups with at most 32 routing choices, then detail pages with at most 48 options and 12,000 criteria characters (a single indivisible action is retained). Every candidate remains reachable. This bounds enumeration size as well as option count; these are application budgets, not an exact tokenizer. The model chooses an existing candidate, never executable code or invented coordinates.
