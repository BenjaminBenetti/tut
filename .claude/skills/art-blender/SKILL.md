...
| Shading | Flat by default; smooth Crescent shells and joint cuffs are intentional. `bpy_kit` primitives are flat by default; organic bug flesh may pass `smooth=True`. `bevel(ob, 0.03)` chamfers armour plates (about 2× the triangles of a plain box; keep it for hero parts). |
...
| Crescent swarmer / lurker / brute | 16 000 / 18 000 / 20 000 | 500 KiB |
| Crescent egg spawner | 16 000 | 500 KiB |
| Tile piece | 60 | 20 KB |
| Building module | 800 | 100 KB |
| Prop | 300 | 60 KB |

Hard cap for anything: 500 KiB (the validator's default). The Executive Director requested detailed Crescent replacements on 2026-09-12; these supersede the original bug budgets. See `docs/design/kits/crescent-bugs.md` for source, validation and the production-animation review.
