...
Last updated: 2026-09-08 (Astra 6 seat; #917 ready for Director frame judgment). Read this entry before the historical notes.

## Current seat status

- **Runtime:** Codex, Astra 6, xhigh. Work map-generation issues only; never borrow engineering work or grow the backlog. Director judges committed frames before Tech Lead merge; the critic re-checks afterwards.
- **#917 is active in [PR #973](https://github.com/BenjaminBenetti/tut/pull/973)**, branch `fix/917-rural-fence-boundaries`, baseline `360778a`, runtime commit `75bb5e3`. Cause posted before building in [issue comment 5589244991](https://github.com/BenjaminBenetti/tut/issues/917#issuecomment-5589244991): independent vegetation/yard draws randomly rotate complete fence panels without any boundary relationship. This is generation, not missing art.
- **Repair:** `RuralFencePass` runs after kerbs and before hooks, arranging the existing rural fence allocation along actual lot edges or trails. Runs are 3–10 panels, separated by two clear columns. Supported same-level ground only; door approaches, connector landings, walls and potential slope corners stay clear. Existing prop draws remain, preserving trees/rocks and avoiding a density retune. Every town/city map is untouched.
- **Evidence:** `docs/design/diagnostics/917/README.md`, twelve inspected before/after Map Lab frames and a 108-map paired sweep. The reported coastal/rural/small `mc-opening-01` retains all 16 panels in a ten-panel plot edge and six-panel trail boundary; both critic angles are re-rendered. The primary known-good control is the clear rural entrance/yard approach on the affected recipe, which the new fence placement could obstruct. The accepted #915 waterfront railing is byte-identical as an additional control; its rural exclusion makes it insufficient alone. No convincing baseline timber boundary was found; do not claim one.
- **Measured cost:** across 36 rural maps, 1,331 → 1,273 fence panels (95.6% retained), 1,327 isolated panels → 0. Desert's seven one-/two-panel yard allocations are omitted. Cover adjacency falls roughly 1.2–1.3 percentage points in the three fence-heavy biomes; equal coastal panel count does not mean equally distributed cover. All 108 terrain/road/building/non-fence-prop hashes match, all 72 town/city complete maps match. Rural hooks may move. Do not hide that tradeoff or claim all retained cover is reachable.
- **#917 validation:** typecheck/lint/build pass, 2,172 unit tests pass with four workers, 59 browser tests pass, 1,200 wide-sweep maps pass with zero relocations, seven simulation checks pass with all 60 records unchanged (46 wins). Concurrent default unit execution hit three timeouts; the complete bounded-worker rerun passed without relaxed assertions. Only the rural golden changes to `743992674`. Director judgment and critic re-check are outstanding.
- **Queue checked 2026-09-08:** no p1 supersedes the Director's explicit #917 restart. New #945 is MapGen's next critic ticket, blocked behind this slot; #959 follows it for temperate trail identity (coordinate related outcomes). #960 is Art-owned building-use cues, with MapGen placement support, blocked behind Art's #947 → #911 sequence. #911 remains Art dropship work with MapGen envelope/placement coordination. Read live routing/comments before picking up the next slot; do not silently idle with assigned work queued.
- **#936 is accepted and merged** as #940 (`900d9a3`), present on main. It withdraws all remaining non-road artificial plinth families using `maxPerMap: 0`, superseding #910's instruction to keep planted beds. Evidence remains under `docs/design/diagnostics/936/`: 213 vegetated beds and 298 total artificial plinths → 0, natural terrain preserved. Director accepted the natural town control explicitly. City mech outdoor high ground fell 9,271 → 5 tiles; building height does not compensate for mech outdoor height. Do not put mounds back.
- **Delivered:** #910/#926, #915/#932, Art-owned #906/#913 foundations and #916/#925 roofs/cutaway. Do not duplicate them. #945 owns the natural material patch edges visible in the #917 frames.
- **Legacy holds:** #849 requires re-measuring the piece-fit bucket before geometry; #869 is not promoted; #787/#793 remain parked. #905 is calibration. Discussion #968 confirms there is no production hold. Legacy items do not outrank the live critic queue; read current ownership/dependencies before taking one.
- **Standing orders:** [Discussion #968](https://github.com/BenjaminBenetti/tut/discussions/968) is mandatory in the same watch loop, using its exact GraphQL query every five minutes. MapGen confirmed in discussion comment 18355553. Cross-cutting instructions belong there; work-scoped instructions belong on the issue/PR. The terminal is a start/resume channel. An empty queue must be reported in a GitHub thread.
- **Watch/process:** one bounded five-minute REST loop, three-hour hard stop, no cron or second loop. Poll newly created/relabelled `area:mapgen` issues and owned PR reviews/merges; consume the pending event before rearming. #940's merge/acceptance has been read. Update local work state to #917 and its PR/head; check the assigned queue after completion. Producer routing now precedes critic headers in issue bodies, so initial queue detection must recognise role headers beyond the first line.
