...
def build_swarmer() -> None:
    ...
    for side in (-1, 1):
        box(f"eye_{'l' if side < 0 else 'r'}", (0.04, 0.02, 0.04), (side * 0.05, -0.54, 0.15), "bug-bio-green")
        box(f"eye2_{'l' if side < 0 else 'r'}", (0.025, 0.02, 0.025), (side * 0.08, -0.52, 0.11), "bug-bio-green")
    ...
    box("vein", (0.02, 0.4, 0.02), (0, 0.05, 0.38), "bug-bio-green")
    for i, y in enumerate((0.3, 0.12, -0.06)):
        cylinder(f"spine_{i}", 0.0, 0.05, 0.14, 4, (0, y, 0.42 - i * 0.02), "bug-bone", rot=(math.radians(-20), 0, 0))
...
def build_lurker() -> None:
    ...
    box("slit", (0.1, 0.02, 0.02), (0, -0.32, 1.2), "bug-bio-magenta")
    box("spine_glow", (0.03, 0.02, 0.3), (0, 0.06, 1.02), "bug-bio-magenta")
    for side in (-1, 1):
        s = "l" if side < 0 else "r"
    ...
    for i in range(3):
        box(f"tail_glow_{i}", (0.02, 0.06, 0.02), (0, 0.24 + i * 0.12, 0.79 - i * 0.05), "bug-bio-magenta")
...
def build_brute() -> None:
    ...
    box("eye_slit", (0.2, 0.02, 0.03), (0, -0.66, 0.76), "bug-bio-green")
