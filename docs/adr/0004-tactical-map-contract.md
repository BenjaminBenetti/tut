...
| I10 | Natural slope shapes have no orphan corners. A one-layer step's lower tile carries a wedge unless it is paved, walled, or under a connector (#847): a wall is what makes an edge man-made, and a walled column — a building's mirrored wall, a parapet on a raised feature — keeps it; graded but unwalled ground (a yard, the city plat) takes its wedge like a hillside. `Tile.slope` is shape only — no connector starts or lands on a slope tile — and `slopeShare` is visual: a step left bare is walked exactly as a sloped one (ADR 0008 §2.5). Pinned in the generation sweep, including that no unpaved, unwalled one-layer step is left bare. |
