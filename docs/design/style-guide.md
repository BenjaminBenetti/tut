...
- **Icons**: 16 and 24 px, single colour, 2 px stroke, SVG. Live under `public/assets/ui/icons/`. Two sets: overworld and shared chrome (#102), and the tactical HUD set (#466) — `end-turn`, `interact`, the `hidden` and `suppressed` statuses, and the stat glyphs `hp`, `ap`, `armor`, `damage`, `range`, `cover-low`, `cover-high`, `elevation`, `ammo`. `ap` deliberately shows two filled pips and one empty: spent against remaining. A third set for the mech bay stat sheet (#673): `accuracy`, `firepower`, `heat`, `weight` — each chosen against a shape the set already spends, because `attack` and `mission` are both crosshairs and `damage` is already a hit, so accuracy is a bullseye with an arrow landing in it and firepower is a shell, the thing thrown rather than the thing landed.
