...
Expanded [real-map navigation evaluations](../experiments/jev-navigation/README.md) now offer 100 real units as destinations, with actor and entity metadata on every call. Across six fully visible 48/72/96 maps and four order types, a tools-only prototype that shares repeated capability definitions reached 24/24 targets with optimal AP; inline metadata reached 19/24, failing five equipment-based orders. The shared format averaged 17,157 input tokens versus 28,235 inline. Jev selects the entity; the game routes one AP and asks again. The viewer contains the crowded maps, exact requests, raw responses and failures. Units are stationary, combat and fog are outside this stress test, and the prototype has not changed the runtime controller.
