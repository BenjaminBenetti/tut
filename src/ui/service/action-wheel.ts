 ...
 *                 ship tile) · Deploy radar · Deploy turret (#1138) · Overwatch · Reload
...
function tilePage(tile: TileCoord, unit: Unit, ctx: WheelContext): WheelPage {
  ...
  // The dish where a scanner may go (#1132), and the turret beside it
  // (#1138). A grenade or a charge is an attack and rides under Attack
  // with the weapons (#1136): the ring used to break them out beside
  // it, and the Executive Director found two places to look for one
  // kind of thing confusing.
  for (const carried of equipmentOf(
    ctx.mission.templates[unit.templateId],
    unit,
    SHIPPED_EQUIPMENT,
  )) {
    if (isDeployable(carried.definition)) {
      items.push(
 ...
 * ring. Closed with the rules' reason when the use is refused.
 */
 ...
 * (#1138), which is put down — each of which keeps its own entry. In
 * the order the template lists it, so the page and the card agree.
 */
function attackKitOf(
  ...
  return equipmentOf(
    ...
    unit,
    SHIPPED_EQUIPMENT,
  ).filter((carried) => !isDeployable(carried.definition));
}
