...
The baseline is main `29639b3e2289b6f42f50f9497c688a1c1e17e0f4` (the exact
checkout hash is recorded in `before/captures.json`). All five baseline views
were rendered twice, in separate browser launches, with byte-identical PNGs.
...
The flat-roof cutaway check uses the existing #943/#947 scene at yaw2, with no
parameter overrides. It compares closed and one-squad controls, then pointer
inspection, overlap and exact closure after the pointer leaves. It also
requires identical bytes in a second browser. Shader state is recorded beside
the frames.

[Kit and mount contract](../../kits/building-frontages.md).
`model-validation.json` records the five exported GLBs: every material primitive
